# AICT データフローガイド (v1.5.1-beta.1)

このドキュメントでは、AI Code Tracker (AICT) v1.5.1-beta.1がどのようにデータを記録し、レポートを生成しているかを詳しく説明します。
...
ユーザーアクション: aict report --since 7d
    ↓
handleRangeReport()
    ├─> validateSinceInput()（v1.5.1-beta.1）
...
AICT v1.5.1-beta.1のデータフローは以下の特徴があります：
...
- ✅ **v1.5.1-beta.1**: stash/restore時のAI帰属保全、TOCTOU修正（ファイルロック改善）、PurgeExpiredCheckpoints API簡素化、テストカバレッジ向上、`--since`入力バリデーション追加
