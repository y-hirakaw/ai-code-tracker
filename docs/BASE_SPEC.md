# AI Code Tracker (AICT) 基本仕様書 v1.4.3
...
10. **入力バリデーション**: `--since`の未知形式を警告、`--format`のエラーメッセージに利用可能フォーマット表示（v1.4.3）
...
- **v1.4.3**: テストカバレッジ向上、`--since`入力バリデーション、`--format`エラーメッセージ改善
