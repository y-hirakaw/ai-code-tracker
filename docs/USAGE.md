...
最新版では詳細メトリクスが常に表示され、Summaryセクションは廃止されました：
...
**変更点（v1.1.2）**:
- ❌ Summaryセクション削除（重複情報のため）
- ❌ 「📈 Detailed Metrics」区切り線削除（よりシンプルに）
- ✅ 詳細メトリクスを常時表示（--detailedフラグ不要）
- ✅ By Authorは「行追加」表示に変更（より直感的）
- ✅ 差分追跡方式により期間内の変更のみを集計
