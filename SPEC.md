...
| 機能 | aict | git-ai |
|------|------|--------|
| **目的** | コード生成率計測に特化 | 包括的なAIコード追跡 |
| **実装言語** | Go | Rust |
| **配布形式** | 単一バイナリ | CLIツール + エージェント統合 |
| **コマンド** | `aict` | `git-ai` (Gitラッパー) |
| **機能範囲** | 最小限（チェックポイント + 統計） | フル機能（blame, trend, PR bot等） |
| **Git統合** | Hooks（オプション） | 完全統合（Gitプロキシ） |
| **対象ユーザー** | 計測重視のチーム | AI開発全般 |

**差別化ポイント:**
- より軽量でシンプル
- Go実装により既存のGoプロジェクトに統合しやすい
- CI/CD統合を第一に考慮
- 独自のデータフォーマット（互換性なし）
...
- 設計の影響を受けたプロジェクト: git-ai (概念のみ参考、実装は独自)