...
// formatCSV formats as CSV
func (f *Formatter) formatCSV(report *PeriodReport) string {
	var builder strings.Builder

	if len(report.DailyStats) > 0 {
		// Daily statistics CSV
		builder.WriteString("Date,AI_Lines,Human_Lines,Total_Lines,AI_Percentage,Human_Percentage,Target_Percentage,Progress\n")
		...
		for _, stat := range report.DailyStats {
			...
			if dailyTotal > 0 {
				aiPercentage = float64(stat.AILines) / float64(dailyTotal) * 100
				humanPercentage = float64(stat.HumanLines) / float64(dailyTotal) * 100
				progress = aiPercentage / f.targetAIPercentage * 100
			}

			builder.WriteString(fmt.Sprintf("%s,%d,%d,%d,%.1f,%.1f,%.1f,%.1f\n",
				stat.Date.Format("2006-01-02"),
				stat.AILines,
				stat.HumanLines,
				dailyTotal,
		...
		builder.WriteString("Period_From,Period_To,AI_Lines,Human_Lines,Total_Lines,AI_Percentage,Human_Percentage,Target_Percentage,Progress\n")
		builder.WriteString(fmt.Sprintf("%s,%s,%d,%d,%d,%.1f,%.1f,%.1f,%.1f\n",
			report.Range.From.Format("2006-01-02"),
			report.Range.To.Format("2006-01-02"),
			report.AILines,
			report.HumanLines,
